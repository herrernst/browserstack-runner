...
exports.Server = function Server(bsClient, workers) {
  ...
  function handleFile(filename, request, response, doNotUseProxy) {
    ...
    var writeResponse = function(err, data) {
      ...
      if (!mimeType) {
        mimeType = 'application/octet-stream'; //default if not mapped
      }
      ...
      if (isText) {
        //.toString() implicitly converts to utf-8
        data = data.toString().replace(matcher, patch);
      }
    ...
    if (!doNotUseProxy && config.test_server) {
      ...
      fs.exists(filename, function(exists) {
        ...
        fs.readFile(filename, null, writeResponse);
      });
    }
  }
